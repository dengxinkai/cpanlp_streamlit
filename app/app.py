...
if st.session_state.input_api:
    ...
    def 中国平安年报查询(input_text):
        ...
        www=index.query(vector=a, top_k=5, namespace='ZGPA_601318', include_metadata=True)
    ...
    def 双汇发展年报查询(input_text):
        ...
        index = pinecone.Index(index_name="kedu")
        a=embeddings.embed_query(input_text)
        www=index.query(vector=a, top_k=5, namespace=namespace, include_metadata=True)
        c = [x["metadata"]["text"] for x in www["matches"]]
        return c
