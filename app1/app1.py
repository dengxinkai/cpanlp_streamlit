...
class GenerativeAgent(BaseModel):
    ...
    def generate_dialogue_response(self, observation: str) -> Tuple[bool, str]:
        ...
        if "GOODBYE:" in result or "GOODBYE：" in result or "再见：" in result or "再见:" in result:
            ...
            self.agent_memory = self.agent_memory + ',' + f"{self.name} 观察到 {observation} 同时说 {farewell}"
        ...
        if "SAY:" in result or "SAY：" in result or "说：" in result or "说:" in result:
            ...
            self.agent_memory = self.agent_memory + ',' + f"{self.name} 观察到 {observation} and 同时说 {response_text}"
