...
with st.sidebar:
    ...
    with st.expander("ChatOpenAI属性设置"):
        temperature = st.slider("`temperature`", 0.01, 0.99, 0.3,help="在OpenAI GPT语言模型中，温度（temperature）是一个用于控制生成文本随机性和多样性的参数。它可以被理解为对下一个词的概率分布进行重新加权的因子，其中较高的温度值将导致更多的随机性和不确定性，而较低的温度值将导致更少的随机性和更高的确定性。通过调整温度值，可以控制生成文本的风格和多样性，以满足特定的应用需求。较高的温度值通常适用于生成较为自由流畅的文本，而较低的温度值则适用于生成更加确定性的文本。")
        frequency_penalty = st.slider("`frequency_penalty`", 0.01, 0.99, 0.3,help="用于控制生成文本中单词重复频率的技术。它会惩罚模型在生成文本时重复使用同一单词的行为，从而鼓励模型生成更多不同的单词，使生成的文本更加多样化和高质量。这个参数的数值越大，模型对单词重复使用的惩罚就越严格，生成文本中出现相同单词的概率就越低；反之，数值越小，生成文本中出现相同单词的概率就越高。")
        presence_penalty = st.slider("`presence_penalty`", 0.01, 0.99, 0.3,help="OpenAI的presence_penalty是用于控制语言生成模型生成文本时对输入提示的重视程度的参数。在生成文本时，模型使用输入提示作为指南，以确保生成的文本与主题相关。但是，如果presence_penalty的值较低，模型生成的文本可能与输入提示非常接近，但缺乏创意或原创性。另一方面，如果将presence_penalty设置为较高的值，模型可能生成更多样化、更具原创性但与输入提示较远的文本。")
        top_p = st.slider("`top_p`", 0.01, 0.99, 0.3,help="用于控制生成文本的多样性。当使用top_p参数进行生成时，模型会在预测下一个词的概率分布中选择概率值最高的一组词，直到这些词的概率总和超过了预设的阈值top_p为止。然后，模型会从这个词集中按照概率分布随机选择一个词作为下一个词，这样可以保证生成的文本不会过于单一。top_p控制了生成文本的多样性，较小的top_p值会让模型选择的词更加确定，生成的文本会更加一致，而较大的top_p值则会让模型选择的词更加多样，生成的文本则更加多样化。")
        model = st.radio("`模型选择`",
                                ("gpt-3.5-turbo",
                                "gpt-4"),
                                index=0)
